...
16.1
  ...
  1. プログラムの保守性とは
    ...
    プログラムの変更容易性や影響範囲の大きさ

  1. プログラムの再利用性

    一度作成したプログラムを別の機会にどの程度容易に使いまわせるか

16.2

  1. UMLのクラス図とシーケンス図はそれぞれプログラムの何を表すか
...
16.3
  ...
  1. テンプレートメソッドパターンを使うと、どのようなメリットがあるか

    抽象クラスと具象クラスにわけるため、共通的な処理が1箇所にまとめられ、変更が必要な場合も1箇所の修正で済む

16.4

  1. ステートパターンを使うと、どのようなメリットがあるか

    状態をクラスとして持つので、拡張性が高い
    状態ごとの処理を局所化できる

16.5

  1. アブストラクトファクトリパターンを使うと、どのようなメリットがあるか

    複数のオブジェクトをまとめて生成する場合でも、呼び出されるオブジェクトのバリエーションが増えても対応できる

    生成対象のオブジェクト群の生成を抽象化するため、バリエーションが増えた場合には新しいFactoryクラスを用意することで、既存のクラスを変更することなく対応できる

16.6

  1. デコレータパターンを使うと、どのようなメリットがあるか

    追加機能を再帰的に呼びだせる構造により、あらかじめ機能を組み合わせたクラスを用意しなくても、必要なときに動的に機能を組み合わせて呼び出すことができる

16.7

  1. オブザーバーパターンを使うとどのようなメリットがあるか

    あるオブジェクトの状態遷移を、別の複数のオブジェクトに対し通知し、通知先のオブジェクトの状態を遷移させる
    通知させるクラスの種類が増えたときに通知元クラスを変更せずに済む

16章の理解度チェック

  1. 複数のクラスに同じ内容のメソッドが散財しているケースではどんなデザインパターンが適用できるか

    テンプレートデザインパターン

  1. 様々な機能の組み合わせが考えられるケースではどんなデザインパターンが適用できるか
    
    デコレータパターン

  1. 状態ごとに処理内容が変わるメソッドがあるケースではどのようなデザインパターンを適用できるか
